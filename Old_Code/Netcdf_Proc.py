...
def Loadinandcut(httpline,latmin,latmax,lonmin,lonmax):
   ...
   ##Appropriate size for your study extent and turn into netcdf4"""
    ...
    #start=time.time()
    file_slice.to_netcdf(Drive+modelname+".nc")
    #rint('Done',(time.time() - start))  
    ##Time is 6:11
    file_slice.close()
...
with open(filepath) as fp:
    lines = fp.readlines()### This is the individual files we need to dl
#print(lines)
...
for i in httplut:
    ...
    print(key)    
    print("Formatting",time.time() - start1)
    ...
    Print_Ready.to_csv(path=Drive+"Outputs/"+modelname+".csv")
    


#nc = Dataset(lines[0])
### Here we parralize the writing of the netcdf files. 
### it is set up for six jobs, the writing is cpu slow, and uses minimal computing.
...
NC_path=(Drive+modelname+".nc")
print(NC_path)
Precip = Dataset(NC_path,"r")
    
print(Precip.variables)


#### Here we filter out the variable in each file 
...
### Individual Cutter for each shape file 
### Note this fails when parralized, owing to RAM exceedance likely due to 
### in holding the cdfs in active mem at the same time.
### 


start=time.time()
### here we format the Netcdf for processing
Template,dates=FormatNetCdf(NC_path, key)
print("FormattingDone",time.time() - start)
### masking and writing matrices
#MaskandWritepart=partial(MaskandWrite, key, Template, dates,"eco2")
#start=time.time()
#shapes=[Shape1,Shape2,Shape3,Shape4]
...
### Remove Netcdf for file managment. 
...
start = time.time()
Array= np.array(file.precipitation)
print(elapsed)
file.precipitation
#ttr=xr.DataArray(file.precipitation)
file.lat
start = time.time()
t=Dataset(file)
elapsed = (time.time() - start)
