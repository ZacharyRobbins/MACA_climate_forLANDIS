...
path='New_Climate_Regions/done/'
Ecos,Modelfiles=ModelParser(Modelname,filepath,2,11,path,shapefiles)
shapefiles=["eco1","eco2","eco3","eco4","eco5","eco6","eco8","eco9",
             "eco10","eco11"]
def ModelParser(Modelname,filepath,EcoregionBeginswith,EcoregionEndswith,path,shapefiles):
   ...
   del Lineread[-1]
   Modelfiles=[y for y in Lineread if Models in y]
   print("Running " + Modelname+ " With "+str(len(Modelfiles))+" Files ")
   print("With data for")
   for i in Modelfiles:
        trimmed_name = i.split('macav2metdata_')[1]
        modelname = trimmed_name.split('_daily')[0]
        print(modelname .split('_')[0])
   return(Ecos,Modelfiles)
